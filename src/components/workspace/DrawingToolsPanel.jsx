    ...
    <div className="flex min-w-0 flex-col rounded-md border border-gg-border/60 bg-gg-workspace/25">
      ...
      {open ? (
        <div
          className={
            contentClassName ??
            'flex min-w-0 flex-row flex-wrap items-center gap-x-1 gap-y-1 border-t border-gg-border/40 px-2 py-2'
          }
        >
          {children}
        </div>
...
/**
 * Sketching primitives and relations for the sidebar Drawing tab.
 *
 * @param {{
 *   tool: string
 ...
 *   arcMode: string
 *   onArcModeChange: (m: string) => void
 *   splineType: string
 *   onSplineTypeChange: (t: string) => void
 *   splineTension: number
 *   onSplineTensionChange: (n: number) => void
 *   splineClosed: boolean
 *   onSplineClosedChange: (b: boolean) => void
 *   splineSegmentsPerSpan: number
 *   onSplineSegmentsPerSpanChange: (n: number) => void
 *   splinePanelOpen: boolean
 *   cutMode?: boolean
 *   onCutModeChange?: (b: boolean) => void
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto overflow-x-hidden p-2 text-[12px] text-gg-text">
      <p className="text-[11px] leading-snug text-gg-muted">
        Primitives, shapes, and sketch relations. Use the canvas toolbar for
        selection, undo, and save.
      </p>
      <div className="flex flex-col gap-2">
        ...
        <SidebarToolSection
          ...
          onToggle={onRibbonSectionToggle}
          contentClassName="flex min-w-0 flex-row flex-wrap items-center gap-x-1 gap-y-1 border-t border-gg-border/40 px-2 py-2"
        >
          {curveTools.map((item) => (
            <ToolBtn
              key={item.id}
          ...
          {tool === TOOL.ARC && onArcModeChange ? (
            <>
              <GroupDivider />
              <div
                className="flex w-full shrink-0 flex-wrap items-center gap-0.5 rounded-md border border-gg-border/70 bg-gg-workspace/40 px-0.5 py-0.5"
                role="group"
                aria-label="Arc mode"
              >
          ...
          onSplineSegmentsPerSpanChange ? (
            <>
              <GroupDivider />
              <div
                className="flex w-full min-w-0 flex-col gap-2 rounded-md border border-gg-border/70 bg-gg-workspace/40 px-2 py-2"
                role="group"
                aria-label="Spline toolbox"
              >
                <span className="text-[9px] font-medium uppercase tracking-wide text-gg-muted">
                  Spline
                </span>
                ...
                <div className="flex min-w-0 flex-col gap-2">
                  <label className="flex min-w-0 flex-1 items-center gap-1 text-[10px] text-gg-muted">
                  ...
                  <label className="flex items-center gap-2 text-[10px] text-gg-muted">
                    <span>Samples</span>
                    <input
                      type="number"
                      min={4}
                      ...
                      onChange={(e) =>
                        onSplineSegmentsPerSpanChange(
                          Math.min(
                            48,
                            Math.max(4, Math.round(Number(e.target.value))),
                          ),
                        )
                      }
        ...
        <SidebarToolSection
          ...
          onToggle={onRibbonSectionToggle}
          contentClassName="flex min-w-0 flex-col gap-1 border-t border-gg-border/40 px-2 py-2"
        >
          <ShapesPanel
            tool={tool}
            onToolChange={onToolChange}
        ...
        <SidebarToolSection
          ...
          onToggle={onRibbonSectionToggle}
          contentClassName="border-t border-gg-border/40 px-2 py-2"
        >
          <RelationsPanel
        ...
        {registeredShapesSlot ? (
          <SidebarToolSection
            ...
            label="Registered"
            open={sections.registered !== false}
            onToggle={onRibbonSectionToggle}
            contentClassName="border-t border-gg-border/40 px-2 py-2"
          >
            {registeredShapesSlot}
